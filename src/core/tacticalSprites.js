...
/**
 * Aircraft chevron sprite oriented by true track (degrees).
...
export function aircraftSpriteDataUrl(trackDeg = 0, military = false, size = 24) {
  const key = `ac-${Math.round(trackDeg)}-${military ? 'm' : 'c'}-${size}`
  ...
  const fill = military ? '#ff6b35' : '#00d4ff'
  const stroke = military ? 'rgba(255, 107, 53, 0.5)' : 'rgba(0, 212, 255, 0.45)'
